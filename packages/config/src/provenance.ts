...
const derived = (ref: string, note: string): ParameterProvenance => ({ status: 'derived', ref, note });
const prov = (note: string, pending?: string): ParameterProvenance =>
  pending === undefined ? { status: 'provisional', note } : { status: 'provisional', note, pending };
...
export const PARAMETER_PROVENANCE: Readonly<Record<string, ParameterProvenance>> = {
  ...
  'flight.climbDescentPenaltyMinutes': d('docs/03 §3.6', 'blockTime: penalización de subida y descenso = 14.'),
  'flight.routeDistanceFactor': d('docs/03 §3.6', 'blockTime y fuelBurn usan distancia × 1,06.'),
  'flight.minimumBlockMinutes': prov('Suelo de tiempo de bloque para rutas muy cortas; evita duraciones absurdas.'),
  ...
  'fuel.priceSigma': prov('Volatilidad del proceso del precio del combustible; docs/04 §4.5 da la forma pero no la sigma.'),

  'demand.scale': prov('Constante de escala del modelo de gravedad. Depende por completo de C-1.', 'C-4'),
  ...
  'demand.segmentShare.business': prov('docs/04 §4.1 describe los tres segmentos pero no su proporción.', 'C-8'),
  'demand.segmentShare.leisure': prov('docs/04 §4.1 describe los tres segmentos pero no su proporción.', 'C-8'),
  'demand.segmentShare.vfr': prov('docs/04 §4.1 describe los tres segmentos pero no su proporción.', 'C-8'),
  'demand.noFlyUtility': prov('docs/04 §4.2 exige la opción "no volar" en el denominador pero no le da valor.', 'C-3'),
  'demand.maxRangeKm': prov('Corte por encima del cual no se modela demanda en la Fase 1 (alcance europeo).'),
  ...
  'logit.business.timeOfDay': prov('docs/04 §4.1 lo ordena como "muy alta" pero no lo cuantifica.', 'C-2'),
  'logit.business.frequency': prov('docs/04 §4.1 lo ordena como "muy alta" pero no lo cuantifica.', 'C-2'),
  ...
  'logit.business.loyalty': prov('docs/04 §4.2 menciona la ventaja del incumbente sin cuantificarla.', 'C-2'),
  ...
  'logit.leisure.frequency': prov('docs/04 §4.1 lo ordena como "baja" pero no lo cuantifica.', 'C-2'),
  ...
  'fares.perKmCents.business': derived('docs/02 §2.5', 'Mismo ajuste que fares.baseCents.business.'),
  'fares.distanceExponent': d('docs/04 §4.3', 'referencePrice: distancia^0,87.'),
  'fares.minPriceFactor': prov('Barrera de seguridad frente a precios absurdos; no es una regla de diseño documentada.'),
  'fares.maxPriceFactor': prov('Barrera de seguridad frente a precios absurdos; no es una regla de diseño documentada.'),
  ...
  'costs.cabinCrewHourlyCents': derived('docs/04 §4.9', 'Mismo reparto que costs.pilotHourlyCents.'),
  ...
  'costs.cateringCentsPerPaxByServiceLevel': prov('docs/04 §4.4 usa cateringCost(serviceLevel) sin dar la escala.'),
  'costs.overheadCentsPerAircraftPerDay': prov('docs/04 §4.4 lista el overhead corporativo como coste fijo sin cuantificarlo.'),
  'costs.maintenanceConditionPenaltyMax': prov('docs/04 §4.4 aplica conditionPenalty() sin definir su rango.'),
  ...
  'fleet.ageReliabilityOnsetYears': d('docs/03 §3.7', 'La fiabilidad decae "lentamente tras 15 años".'),
  'fleet.ageReliabilityPerYear': prov('docs/03 §3.7 describe la caída por edad sin dar la pendiente.'),
  'fleet.conditionReliabilityFloor': d('docs/03 §3.7', 'reliability × (0,72 + 0,28 × condición/100).'),
  'fleet.conditionReliabilitySpan': d('docs/03 §3.7', 'reliability × (0,72 + 0,28 × condición/100).'),
  'fleet.deferredCheckPenalty': prov('docs/03 §3.7 dice que diferir "penaliza fuerte" sin dar el coeficiente.'),
  ...
  'fleet.checks.A.costPerSeatCents': derived('docs/03 §3.7', 'A = 12.000 € para un narrowbody de 180 plazas, expresado por asiento para escalar entre tipos.'),
  ...
  'fleet.checks.B.costPerSeatCents': derived('docs/03 §3.7', 'B = 45.000 € para 180 plazas, expresado por asiento.'),
  ...
  'fleet.checks.C.costPerSeatCents': derived('docs/03 §3.7', 'C = 380.000 € para 180 plazas, expresado por asiento.'),
  ...
  'fleet.checks.D.costPerSeatCents': derived('docs/03 §3.7', 'D = 2.100.000 € para 180 plazas, expresado por asiento.'),
  'fleet.checks.D.conditionRestored': d('docs/03 §3.7', 'Tabla de checks: D restaura la condición a ~95.'),

  'delays.technicalMaxMinutes': prov('docs/03 §3.6 lista el retraso técnico como sumando sin dar su magnitud.'),
  'delays.technicalConditionThreshold': prov('Condición por debajo de la cual empieza a aparecer retraso técnico.'),
  'delays.onTimeThresholdMinutes': d('docs/03 §3.3', 'Un vuelo se considera retrasado por encima de 15 minutos.'),
...
export interface ProvenanceSummary {
  ...
  readonly provisional: readonly { readonly path: string; readonly note: string; readonly pending?: string }[];
}
